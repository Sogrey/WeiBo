...
public class AccountAPI extends AbsOpenAPI {
    ...
    public void getPrivacy(RequestListener listener) {
        requestAsync(SERVER_URL_PRIX+"/get_privacy.json",new WeiboParameters(mAppKey),
     ...
     *         区域范围，区ID
     * @param schoolType
     *         学校类型，可为以下几种： 1：大学，2：高中，3：中专技校，4：初中，5：小学
    ...
    public void schoolList(
            int province,int city,int area,int schoolType,CAPITAL capital,String keyword,
    ...
    public void rateLimitStatus(RequestListener listener) {
        requestAsync(SERVER_URL_PRIX+"/rate_limit_status.json",new WeiboParameters(mAppKey),
                     HTTPMETHOD_GET,listener);
    }
    ...
    public void getUid(RequestListener listener) {
        requestAsync(SERVER_URL_PRIX+"/get_uid.json",new WeiboParameters(mAppKey),HTTPMETHOD_GET,
                     listener);
    }
    ...
    public void endSession(RequestListener listener) {
        requestAsync(SERVER_URL_PRIX+"/end_session.json",new WeiboParameters(mAppKey),
